...
const toTitleCase = (arr) => {
  ...
  let answer = arr.map(element => {
    let character = element[0].toUpperCase();
    let newWord = element.slice(1);
    return character + newWord;
  });
  return answer;
};
...
Instead of trying to write crazy for loops to automate checking the rows, columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the values of the array at those locations. For instance answer(row1, col1, row2, col2, row3, col3).
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const answer = (row1, col1, row2, col2, row3, col3) => {
    return board[row1][col1] !== '' &&
      board[row1][col1] === board[row2][col2] &&
      board[row2][col2] === board[row3][col3];
  };

  if (answer(0, 0, 0, 1, 0, 2)) return true;
  if (answer(1, 0, 1, 1, 1, 2)) return true;
  if (answer(2, 0, 2, 1, 2, 2)) return true;

  if (answer(0, 0, 1, 0, 2, 0)) return true;
  if (answer(0, 1, 1, 1, 2, 1)) return true;
  if (answer(0, 2, 1, 2, 2, 2)) return true;

  if (answer(0, 0, 1, 1, 2, 2)) return true;
  if (answer(0, 2, 1, 1, 2, 0)) return true;
