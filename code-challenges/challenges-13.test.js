...
const longestString = (arr) => {
  ...
  for (let element in arr) {
    if (arr[element].length > answer.length) {
      answer = arr[element];
...
const firstLetters = (arr) => {
  // Solution code here...
  let anwser = arr.map(string => string.charAt(0));
  return anwser;
};
...
const findHappiness = (arr) => {
  ...
  let newArray = [];
  arr.forEach(string => {
    (string.includes(':)')) ? newArray.push(string) : null;
  });
  return newArray;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((element, char) => {
    return (char % 2) === 1;
  }).join('');
};
...
const allHappy = (arr) => {
  ...
  arr.forEach(string => {
    if(!string.includes(key)) return hasIt = false;
  });
  return hasIt;
};
